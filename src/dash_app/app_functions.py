...
def boundary_to_points(boundary_points, fig_shape):
	## check all pixels in rectangle bounding selection to see if they are within selection
	bottom_left = np.floor(np.min(boundary_points, axis=0)).astype(int)
	upper_right = np.ceil(np.max(boundary_points, axis=0)).astype(int)
	...
	pixels_in_selection = in_hull(pixel_grid_points, boundary_points)
	return pixel_grid_points[pixels_in_selection==True]
...
def forward_prop_cells(cell1, cell2, fig_shape):

	## convert cell2 dict to a mask of the image space, which we will match to cells1

	points_set = set()
	for k in cell2.keys():
		if 'points' in cell2[k].keys():
			points_set.update([tuple(x) for x in cell2[k]['points']])
		else:
			boundary_points = cell2[k]['boundary']
			points = boundary_to_points(boundary_points, fig_shape)
			points_set.update([tuple(x) for x in points])
	...
	for k in cell1.keys():
		cell_k = cell1[k]
		if 'points' in cell_k.keys():
			points_set_k = set([tuple(x) for x in cell_k['points']])
		else:
			boundary_points = cell_k['boundary']
			points = boundary_to_points(boundary_points, fig_shape)
			points_set_k = set([tuple(x) for x in points])
